...
import 'package:flutter_application_2/AddnamePage.dart';
...
class OTP_Page extends StatefulWidget {
  ...
  OTP_Page({Key? key, required this.phoneNumber, required this.isSignin})
      : super(key: key);
...
class _OTP_PageState extends State<OTP_Page> {
        ...
        body: SafeArea(
          child: ListView(
            children: [
              ...
              Padding( padding: const EdgeInsets.only(bottom: 55.0, top: 50),
                  ...
                  onPressed: () {
                    if (otpControllers.any((controller) => controller.text.isEmpty)) {
                      setState(() {
                        ...
                        isAnyBoxEmpty = true;
                        errorMessage = 'يرجى ملء جميع المربعات *'; // error message for empty boxes
                    ...
                    if (enteredOTP == expectedOTP) {
                      ...
                      // determine the navigation after the OTP check
                      if (widget.isSignin) { 

                        navigateToHomePage(); // if the user has an account before , navigate to (HomePage)
                      } else {

                        navigateToAddNamePage(); // if the user signing up , navigate to (AddNamePage)
                      }
                    } else {
                      setState(() {
  ...
  Future<void> phoneAuth(phoneNumber) async {
    ...
    await FirebaseAuth.instance.verifyPhoneNumber( 
      ...
      // when the verification is completed , it signs in the user with the provided credential and navigate To AddNamePage
      verificationCompleted: (PhoneAuthCredential credential) async {
        await auth.signInWithCredential(credential);
        navigateToAddNamePage();
      },
      // when the verification process fails , prints a message
  ...
  void navigateToAddNamePage() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) =>
            AddnamePage(phoneNumber: phoneNumber), // Pass the phone number
      ),
    );
  }
